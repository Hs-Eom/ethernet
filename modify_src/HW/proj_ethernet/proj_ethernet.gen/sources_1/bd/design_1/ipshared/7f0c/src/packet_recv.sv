...
module packet_recv 
  #(
   ...
   logic [7:0]	    fireset_n_packet_count;

   localparam	    FIreset_n_PACKET_IGNORE = 0;
   ...
   always @(posedge clk)                                             
	...
	if (!reset_n)                                                    
	  ...
	  begin                 
	   ...
	   if (packet_done & fireset_n_packet_count < FIreset_n_PACKET_IGNORE) begin
	      fireset_n_packet_count <= fireset_n_packet_count + 1;
   ...
   assign preamble_sfd_buffer_next[63:62] = (~reset_n) ? 0 : rxd_z[2];
   assign preamble_sfd_buffer_next[61:0] = (~reset_n) ? 64'b0 : preamble_sfd_buffer[63:2];
